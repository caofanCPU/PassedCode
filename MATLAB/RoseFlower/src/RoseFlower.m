...
draw_main(450,90);
pause(0.2);
b5 = text(205,20,'HelloWorld','fontsize',20,'color','m');
pause(5);
b2 = text(320,325,'Here is a roseflower...','fontsize',20,'color','m');
pause(2);
b0 = text(220,355,'Send to you in your every happyday！','fontsize',20,'color','m');
pause(0);
return;
...
figure('menubar','none','numbertitle','off','name','Beaytiful RoseFlower');
hold on;
for j = 1:1:length(ellipsedata)
    for i = 1:1:size(ellipsedata{j},1)
        rectangle('Position',[x+ellipsedata{j}(i,1)-ellipsedata{j}(i,5),...
                  y+ellipsedata{j}(i,2)-ellipsedata{j}(i,6),...
                  2*ellipsedata{j}(i,5),2*ellipsedata{j}(i,6)],...
                  'Curvature',[1,1],'FaceColor',ss{j});
    end
    if j<4
        for i = 1:1:size(arcdata{j},1)
            draw_arc(x+arcdata{j}(i,1),y+arcdata{j}(i,2),arcdata{j}(i,3),...
                     arcdata{j}(i,4),arcdata{j}(i,5),s0{j});  %
        end
    end
    for i = 1:1:size(rose_e{j},1)
        plot_rose_e(rose_e{j},j);  %
    end
end
for j = 1:1:3;
    zz = [x-98  y+100+j 255 371 100 80 ;
...
%
function draw_branch(c_B)
for k =1:1:size(c_B,1)
    t  = (c_B(k,3):c_B(k,4))*pi/180-pi;
    x1 = c_B(k,1) + c_B(k,5)*cos(t);
    x1 = 2*c_B(k,1) - x1;
    y1 = c_B(k,2) + c_B(k,6)*sin(t);
    plot(x1,y1,'g','linewidth',2);
end
title('Beautiful RoseFlower');
return;